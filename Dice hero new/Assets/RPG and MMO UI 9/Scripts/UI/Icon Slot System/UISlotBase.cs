	...
	public class UISlotBase : UIBehaviour, IEventSystemHandler, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler, IPointerClickHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IDropHandler {
		...
		public virtual bool Assign(Object source)
		{
			if (source is UISlotBase)
			{
                ...
                UIItemSlot sSlot = source as UIItemSlot;
		...
		public virtual void OnDrop(PointerEventData eventData)
		{
            ...
            if (!this.IsAssigned())
            {
                ...
                if (!source.isStatic)
                {
                    ...
                    /*if (source1.GetItemInfo().IsStackable || source1==null)
                    {
                        source1.transform.GetChild(3).gameObject.SetActive(false);
                        this.transform.GetChild(3).gameObject.SetActive(true);
                        StackNum = this.transform.GetChild(3).GetComponentInChildren<Text>();
                        StackNum.text = source1.GetItemInfo().Stack.ToString();
			...
			else
			{
				...
				{
                    Debug.Log(this.CanSwapWith(source));
                    Debug.Log(source.CanSwapWith(this));
		...
		{
            Debug.Log("perform SlotSwap on BASE");
			// Get the source slot
